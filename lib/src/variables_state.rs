...
impl VariablesState {
    ...
    fn resolve_variable_pointer(&self, var_pointer: &VariablePointerValue) -> Arc<Value> {
        ...
        Arc::new(Value::new_variable_pointer(
    ...
    pub fn set(&mut self, variable_name: &str, value_type: ValueType) -> Result<bool, StoryError> {
        ...
        let notify = self.set_global(variable_name, Arc::new(val));
    ...
    fn set_global(&mut self, name: &str, value: Arc<Value>) -> bool {
        let mut old_value: Option<Arc<Value>> = None;
        ...
        if old_value.is_none() || !Arc::ptr_eq(old_value.as_ref().unwrap(), &value) {
    ...
    pub fn get_variable_with_name(&self, name: &str, context_index: i32) -> Option<Arc<Value>> {
    ...
    fn value_at_variable_pointer(&self, pointer: &VariablePointerValue) -> Option<Arc<Value>> {
