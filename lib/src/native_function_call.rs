...
impl NativeFunctionCall {
    ...
    pub(crate) fn call(
        &self,
        params: Vec<Arc<dyn RTObject + Sync + Send>>,
    ...
    fn call_binary_list_operation(
        &self,
        params: &[Arc<dyn RTObject + Sync + Send>],
    ) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
                ...
                || Value::get_list_value(params[1].as_ref()).is_none())
        {
            ...
            return Ok(Arc::new(Value::new_bool(result)));
        }
    ...
    fn coerce_values_to_single_type(
        &self,
        params: Vec<Arc<dyn RTObject + Sync + Send>>,
    ) -> Result<Vec<Arc<Value>>, StoryError> {
        let mut dest_type = 1; // Int
        let mut result: Vec<Arc<Value>> = Vec::new();
        ...
        for obj in params.iter() {
            if let Some(v) = obj.as_ref().as_any().downcast_ref::<Value>() {
                match v.cast(dest_type)? {
                    Some(casted_value) => result.push(Arc::new(casted_value)),
                    None => {
    ...
    fn and_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::Int(op1) => match params[1].value {
                ValueType::Int(op2) => Ok(Arc::new(Value::new_bool(*op1 != 0 && op2 != 0))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_bool(*op1 != 0.0 && op2 != 0.0))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(
    ...
    fn greater_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_bool(*op1 > op2))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(op1.greater_than(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn less_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(op1.less_than(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn greater_than_or_equals_op(
        ...
        match &params[0].value {
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => {
                    Ok(Arc::new(Value::new_bool(op1.greater_than_or_equals(op2))))
                }
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
    ...
    fn less_than_or_equals_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(op1.less_than_or_equals(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn subtract_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_float(*op1 - op2))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_list(op1.without(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn add_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Int(op1) => match params[1].value {
                ValueType::Int(op2) => Ok(Arc::new(Value::new_int(op1 + op2))),
                _ => Err(StoryError::InvalidStoryState(
            ...
            ValueType::String(op1) => match &params[1].value {
                ValueType::String(op2) => {
                    ...
                    Ok(Arc::new(Value::new_string(&sb)))
                }
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_list(op1.union(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn pow_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match params[0].value {
            ...
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_float(op1.powf(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn or_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Bool(op1) => match params[1].value {
                ValueType::Bool(op2) => Ok(Arc::new(Value::new_bool(*op1 || op2))),
                _ => Err(StoryError::InvalidStoryState(
    ...
    fn not_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Int(op1) => Ok(Arc::new(Value::new_bool(*op1 == 0))),
            ValueType::Float(op1) => Ok(Arc::new(Value::new_bool(*op1 == 0.0))),
            ValueType::List(op1) => Ok(Arc::new(Value::new_int(match op1.items.is_empty() {
                true => 1,
                false => 0,
            }))),
            _ => Err(StoryError::InvalidStoryState(
    ...
    fn min_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Int(op1) => match params[1].value {
                ValueType::Int(op2) => Ok(Arc::new(Value::new_int(i32::min(*op1, op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_float(f32::min(*op1, op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn max_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Int(op1) => match params[1].value {
                ValueType::Int(op2) => Ok(Arc::new(Value::new_int(i32::max(*op1, op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::Float(op1) => match params[1].value {
                ValueType::Float(op2) => Ok(Arc::new(Value::new_float(f32::max(*op1, op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn equal_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(op1.eq(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
            ValueType::DivertTarget(op1) => match &params[1].value {
                ValueType::DivertTarget(op2) => Ok(Arc::new(Value::new_bool(op1.eq(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn mod_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match params[0].value {
            ValueType::Int(op1) => match params[1].value {
                ValueType::Int(op2) => Ok(Arc::new(Value::new_int(op1 % op2))),
                _ => Err(StoryError::InvalidStoryState(
    ...
    fn has(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::String(op1) => match &params[1].value {
                ValueType::String(op2) => {
                    Ok(Arc::new(Value::new_bool(op1.string.contains(&op2.string))))
                }
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            ...
            ValueType::List(op1) => match &params[1].value {
                ValueType::List(op2) => Ok(Arc::new(Value::new_bool(op1.contains(op2)))),
                _ => Err(StoryError::InvalidStoryState(
                    "Operation not available for type.".to_owned(),
                )),
            },
    ...
    fn hasnt(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::String(op1) => match &params[1].value {
                ValueType::String(op2) => {
                    Ok(Arc::new(Value::new_bool(!op1.string.contains(&op2.string))))
    ...
    fn value_of_list_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::List(op1) => match op1.get_max_item() {
                ...
                None => Ok(Arc::new(Value::new_int(0))),
            },
            _ => Err(StoryError::InvalidStoryState(
                "Operation not available for type.".to_owned(),
            )),
    ...
    fn all_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::List(op1) => Ok(Arc::new(Value::new_list(op1.get_all()))),
            _ => Err(StoryError::InvalidStoryState(
                "Operation not available for type.".to_owned(),
            )),
        }
    ...
    fn int_op(&self, params: &[Arc<Value>]) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
        match &params[0].value {
            ValueType::Int(op1) => Ok(Arc::new(Value::new_int(*op1))),
            ValueType::Float(op1) => Ok(Arc::new(Value::new_int(*op1 as i32))),
            _ => Err(StoryError::InvalidStoryState(
                "Operation not available for type.".to_owned(),
            )),
        }
