...
pub fn jtoken_to_runtime_object(
    ...
    match token {
        ...
        serde_json::Value::Bool(value) => Ok(Arc::new(Value::new_bool(value.to_owned()))),
        serde_json::Value::Number(_) => {
            if token.is_i64() {
                let val: i32 = token.as_i64().unwrap().try_into().unwrap();
                Ok(Arc::new(Value::new_int(val)))
            } else {
                let val: f32 = token.as_f64().unwrap() as f32;
                Ok(Arc::new(Value::new_float(val)))
            }
        }
        ...
        serde_json::Value::String(value) => {
            ...
            if first_char == '^' {
                return Ok(Arc::new(Value::new_string(&str[1..])));
            } else if first_char == '\n' && str.len() == 1 {
                return Ok(Arc::new(Value::new_string("\n")));
            }

            // Glue
            if "<>".eq(str) {
                return Ok(Arc::new(Glue::new()));
            }

            if let Some(control_command) = ControlCommand::new_from_name(str) {
                return Ok(Arc::new(control_command));
            }
            ...
            if let Some(native_function_call) = NativeFunctionCall::new_from_name(call_str) {
                return Ok(Arc::new(native_function_call));
            }

            // Void
            if "void".eq(str) {
                return Ok(Arc::new(Void::new()));
            }
        ...
        serde_json::Value::Object(obj) => {
            ...
            if let Some(prop_value) = prop_value {
                return Ok(Arc::new(Value::new_divert_target(
            ...
            if let Some(v) = prop_value {
                ...
                let var_ptr = Arc::new(Value::new_variable_pointer(variable_name, contex_index));
            ...
            if let Some(cp) = prop_value {
                ...
                return Ok(Arc::new(ChoicePoint::new(
                    flags as i32,
            ...
            if let Some(name) = prop_value {
                return Ok(Arc::new(VariableReference::new(name.as_str().unwrap())));
            }

            let prop_value = obj.get("CNT?");
            if let Some(v) = prop_value {
                return Ok(Arc::new(VariableReference::from_path_for_count(
                    v.as_str().unwrap(),
                )));
            }
            ...
            if is_var_ass {
                ...
                let var_ass = Arc::new(VariableAssignment::new(
                    var_name,
                    is_new_decl,
                    is_global_var,
                ));
            ...
            prop_value = obj.get("#");
            if let Some(prop_value) = prop_value {
                return Ok(Arc::new(Tag::new(prop_value.as_str().unwrap())));
            }
            ...
            if let Some(pv) = prop_value {
                ...
                return Ok(Arc::new(Value::new_list(raw_list)));
            }
...
fn jarray_to_container(
    ...
    let mut named_only_content: HashMap<String, Arc<Container>> = HashMap::new();
...
pub fn jarray_to_runtime_obj_list(
    jarray: &Vec<serde_json::Value>,
    skip_last: bool,
) -> Result<Vec<Arc<dyn RTObject + Sync + Send>>, StoryError> {
    ...
    let mut list: Vec<Arc<dyn RTObject + Sync + Send>> = Vec::with_capacity(jarray.len());
...
fn jobject_to_choice(obj: &Map<String, serde_json::Value>) -> Result<Arc<dyn RTObject + Sync + Send>, StoryError> {
    ...
    Ok(Arc::new(Choice::new_from_json(
        path_string_on_choice,
...
) -> Result<HashMap<String, Arc<Value>>, StoryError> {
    let mut dict: HashMap<String, Arc<Value>> = HashMap::new();
