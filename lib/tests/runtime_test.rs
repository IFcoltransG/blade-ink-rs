use core::panic;
use std::{sync::RwLock, error::Error, sync::Arc};
...
fn external_function() -> Result<(), Box<dyn Error>> {
    ...
    story.bind_external_function("externalFunction", Arc::new(RwLock::new(ExtFunc1 {})), true)?;
...
fn external_function_zero_arguments() -> Result<(), Box<dyn Error>> {
    ...
    story.bind_external_function("externalFunction", Arc::new(RwLock::new(ExtFunc2 {})), true)?;
...
fn external_function_one_arguments() -> Result<(), Box<dyn Error>> {
    ...
    story.bind_external_function("externalFunction", Arc::new(RwLock::new(ExtFunc3 {})), true)?;
...
fn external_function_coerce_test() -> Result<(), Box<dyn Error>> {
    ...
    story.bind_external_function("externalFunction", Arc::new(RwLock::new(ExtFunc4 {})), true)?;
...
fn variable_observers_test() -> Result<(), Box<dyn Error>> {
    ...
    story.observe_variable("x", Arc::new(RwLock::new(VObserver { expected_value: 5 })))?;
